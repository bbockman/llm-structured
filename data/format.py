...
# Filter and map with keep_in_memory=True
ds = ds.filter(
...
# tokenizer.save_pretrained("/mnt/xd/ml/hf/tokenizers/synth_tok")
...
ds = ds.filter(lambda x: len(x['text']) <= MAX_LENGTH, num_proc=12, keep_in_memory=True)
...
# DataLoader pulls from stream in batches
